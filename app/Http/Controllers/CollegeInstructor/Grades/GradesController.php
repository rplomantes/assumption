    ...
    function print_grade($school_year, $period, $schedule_id,$course_code=null) {
        if (Auth::user()->accesslevel == env('INSTRUCTOR') || Auth::user()->accesslevel == env('DEAN') || Auth::user()->accesslevel == env('REG_COLLEGE')) {
            ...
            if ($schedule_id == "no_sched") {
                ...
                \App\Http\Controllers\Admin\Logs::log("Print Grade of subject $course_code PDF");
                $pdf = PDF::loadView('college_instructor.print_grade_no_sched', compact('courses_id', 'schedule_id', 'course_name', 'instructor', 'school_year', 'period','course_code'));
                $pdf->setPaper(array(0, 0, 612.00, 792.0));
                return $pdf->stream("student_list_.pdf");
            } else {
