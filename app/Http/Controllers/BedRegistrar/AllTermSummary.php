...
class AllTermSummary extends Controller
{
    ...
    function export_all_term_summary($level, $strand, $section, $school_year, $period = null) {
        ...
        ob_end_clean();
        Excel::create('All Term Summary', function($excel) use ($level, $strand, $section, $school_year,$period, $lists, $subject_heads) {
            $excel->setTitle("All Term Summary");

            $excel->sheet('All Term Summary', function ($sheet) use ($level, $strand, $section, $school_year, $period, $lists, $subject_heads) {
                $sheet->loadView('reg_be.ajax.all_term_view_list', compact('school_year', 'level', 'section', 'period', 'strand', 'lists', 'subject_heads'));
            });
        })->download('xlsx');
    }
}
