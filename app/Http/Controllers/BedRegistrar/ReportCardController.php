...
//                $get_subjects = \App\GradeBasicEd::where('idno', $idno)->where('school_year', $school_year)->where('period',$period)->orderBy('report_card_grouping', 'desc')->orderBy('sort_to','asc')->get();
                ...
                if ($period == "2nd Semester") {
                    ...
                    $pdf = PDF::loadView('reg_be.report_card.print_report_card_individually_shs_1st_semester', compact('idno', 'school_year', 'absents', 'user', 'status', 'adviser', 'get_subjects', 'get_subjects_heads', 'period', 'school_year', 'display_type'));
                }

                $pdf->setPaper(array(0, 0, 720, 576));
                return $pdf->stream("report_card_$idno'_'$school_year'_'$period.pdf");
            } elseif ($status->level == 'Pre-Kinder' || $status->level == 'Kinder') {
                ...
                $pdf = PDF::loadView('student.report_card.view_narrative_report', compact('idno', 'user', 'status', 'adviser', 'narrative_report', 'school_year'));
                $pdf->setPaper(array(0, 0, 720, 576));
