...
class AjaxSetUpController extends Controller
{
    ...
    function getsubsidiary() {
        if (Request::ajax()) {
            ...
            if ($department == "College Department") {
                $subsidiarys = DB::select("SELECT l.subsidiary FROM ledgers l, (SELECT idno FROM college_levels WHERE school_year = '$school_year' AND period = '$period') c WHERE l.idno = c.idno AND l.category_switch IN (1,2,3,4,6,11,12,13,14,16) AND l.school_year = '$school_year' AND period = '$period' GROUP BY l.subsidiary");
