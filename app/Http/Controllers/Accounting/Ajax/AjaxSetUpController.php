    ...
    function getsetupsummary(){
        if(Request::ajax()){
            ...
            if ($department == "College Department") {
                $ledgers = DB::select("SELECT l.accounting_code, l.subsidiary, SUM(l.amount) AS amount, SUM(l.discount) AS discount FROM ledgers l, (SELECT idno FROM college_levels WHERE school_year = '$school_year' AND period = '$period' AND (status = '3' or status = '4')) c WHERE l.idno = c.idno AND l.amount > 0 AND ((l.category_switch < 14 AND l.category_switch > 10) or l.category_switch < 4) AND l.school_year = '$school_year' AND period = '$period' GROUP BY l.subsidiary, l.accounting_code");
                $srfs = DB::select("SELECT l.accounting_code, l.category, SUM(l.amount) AS amount, SUM(l.discount) AS discount FROM ledgers l, (SELECT idno FROM college_levels WHERE school_year = '$school_year' AND period = '$period' AND (status = '3' or status = '4')) c WHERE l.idno = c.idno AND l.amount > 0 AND ((l.category_switch = 14 OR l.category_switch = 4) and l.category = 'SRF') AND l.school_year = '$school_year' AND period = '$period' GROUP BY l.category, l.accounting_code");
                $tuitions = DB::select("SELECT l.accounting_code, l.subsidiary, SUM(l.amount) AS amount, SUM(l.discount) AS discount FROM ledgers l, (SELECT idno FROM college_levels WHERE school_year = '$school_year' AND period = '$period' AND (status = '3' or status = '4')) c WHERE l.idno = c.idno AND l.amount > 0 AND (l.category_switch = 6 or l.category_switch = 16) AND l.school_year = '$school_year' AND period = '$period' GROUP BY l.subsidiary, l.accounting_code");
