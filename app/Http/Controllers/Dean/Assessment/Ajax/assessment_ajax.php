...
namespace App\Http\Controllers\Dean\Assessment\Ajax;
...
class assessment_ajax extends Controller {
    ...
    function get_section() {
        if (Request::ajax()) {
            ...
            $sections = \App\CourseOffering::distinct()->where('school_year', $school_year)->where('period', $period)->where('level', $level)->where('program_code', $program_code)->get(['section']);
            ...
            foreach ($sections as $section) {
                $display = $display . "<option>" . $section->section . "</option>";
            }
    ...
    function get_offering_per_search() {
        if (Request::ajax()) {
            ...
            return view('dean.assessment.ajax.get_course_offering_per_search', compact('courses_offering'));
        }
    }
