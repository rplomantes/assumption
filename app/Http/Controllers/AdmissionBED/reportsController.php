...
class reportsController extends Controller
{
    ...
    function pre_registered($date_start, $date_end) {
        if (Auth::user()->accesslevel == env("ADMISSION_BED") || Auth::user()->accesslevel == env("ADMISSION_SHS")) {
            if(Auth::user()->accesslevel== env("ADMISSION_BED")){
                $auth_type =  "Basic Education";
            }else{
                $auth_type = "Senior High School";
            }
            $non_paid = \Illuminate\Support\Facades\DB::connection('mysql2')->select("select * from pre_registrations where is_complete = 0 and created_at >= '$date_start 00:00:00' and  created_at <= '$date_end 23:59:59' and applying_for = '$auth_type'");
            $paid = \Illuminate\Support\Facades\DB::connection('mysql2')->select("select * from pre_registrations where is_complete = 1 and date_completed >= '$date_start' and  date_completed <= '$date_end' and applying_for = '$auth_type'");
    ...
    function for_approval($date_start, $date_end) {
        if (Auth::user()->accesslevel == env("ADMISSION_BED") || Auth::user()->accesslevel == env("ADMISSION_SHS")) {
            ...
            $for_approval_sched = \App\Status::where('statuses.status', env('FOR_APPROVAL'))->where('statuses.academic_type', "$auth_type")->where('date_application_finish', '>=', $date_start)->where('date_application_finish', '<=', $date_end)->join('users', 'users.idno','=','statuses.idno')->orderBy('users.lastname')->get(['users.idno', 'users.firstname','users.lastname','users.middlename','statuses.level']);
    ...
    function approved($date_start, $date_end) {
        if (Auth::user()->accesslevel == env("ADMISSION_BED") || Auth::user()->accesslevel == env("ADMISSION_SHS")) {
            ...
            $for_approval_sched = \App\Status::where('statuses.status', 0)->where('statuses.academic_type', "$auth_type")->where('date_admission_finish', '>=', $date_start)->where('date_admission_finish', '<=', $date_end)->join('users', 'users.idno','=','statuses.idno')->orderBy('users.lastname')->get(['users.idno', 'users.firstname','users.lastname','users.middlename','statuses.level','date_admission_finish']);
    ...
    function regrets($date_start, $date_end) {
        if (Auth::user()->accesslevel == env("ADMISSION_BED") || Auth::user()->accesslevel == env("ADMISSION_SHS")) {
            ...
            $regret_finals = \App\Status::where('statuses.status', env('REGRET_FINAL'))->where('statuses.academic_type', "$auth_type")->where('date_admission_finish', '>=', $date_start)->where('date_admission_finish', '<=', $date_end)->join('users', 'users.idno','=','statuses.idno')->orderBy('users.lastname')->get(['users.idno', 'users.firstname','users.lastname','users.middlename','statuses.level','date_admission_finish']);
            $regret_retreives = \App\Status::where('statuses.status', env('REGRET_RETREIVE'))->where('statuses.academic_type', "$auth_type")->where('date_admission_finish', '>=', $date_start)->where('date_admission_finish', '<=', $date_end)->join('users', 'users.idno','=','statuses.idno')->orderBy('users.lastname')->get(['users.idno', 'users.firstname','users.lastname','users.middlename','statuses.level','date_admission_finish']);
...
//                   ->get();
           ...
           $reservations = \App\Reservation::where('reservation_type', 1)
                   ...
                   ->join('statuses', 'statuses.idno', '=', 'reservations.idno')
                   ->join('users', 'users.idno','=','reservations.idno')
                   ->where('statuses.academic_type', "$auth_type")
                   ->where('is_new', '1')
                   ->where('date_admission_finish', '!=', NULL)
