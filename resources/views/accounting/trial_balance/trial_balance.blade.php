...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-6">
            <h3 class="display">Trial Balance</h3>
        </div>
    </div>
    <div class="col-md-12">
        <div class='row'>
            <form action="{{ url('/print/trial_balance') }}" method="post">
                {{csrf_field()}}
                <div class='col-md-4'>
                    <div class="form-group">
                        <label class="form-label">Input Transaction Date</label>
                        <div class="input-group">
                            <button type="button" class="btn btn-default pull-left" id="datepicker">
                                <span>
...
$(document).ready(function () {
   $('#example').DataTable();
    ...
    $('#datepicker').daterangepicker(
      ...
      function (start, end) {
        $('#datepicker span #range').html(start.format('YYYY-MM-DD') + ' , ' + end.format('YYYY-MM-DD'));
        x=$('#range').html();
        splitdate=x.split(',');
        todate=splitdate[1];
