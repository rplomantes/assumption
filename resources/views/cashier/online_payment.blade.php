...
@section('maincontent')
 ...
 <div class="container-fluid">
     ...
     <table id="example1" class="table table-responsive table-hover">
         <thead>
             <tr><th>Date</th><th>Receipt No</th><th>Receive From</th><th>Online Payment</th><th>Reference No.</th><th>Response ID</th><th>Amount</th><th>Status</th><th>View</th></tr>
         </thead>
         <tbody>
             @if(count($payments)>0)
                @foreach($payments as $payment)
                    ...
                    <td>
                        ...
                        {{$payment->receipt_no}}
                        @else
                        <a href="javascript:void(0)" data-toggle="modal" data-target="#modal-default"  onclick="issue_or('{{$payment->id}}')">
                            Issue OR Number
                        </a>
                        @endif
                    </td>
...
<script>
    $(document).ready(function(){
      ...
      $("#view-button").on('click',function(e){
        document.location="{{url('/cashier',array('online_payment'))}}" + "/" + $("#date_from").val() + "/" + $("#date_to").val();
    });
