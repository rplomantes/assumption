...
@section('maincontent')
 ...
 <div class="container-fluid">
     ...
     <table id="example1" class="table table-responsive table-hover">
         <thead>
             <tr><th>Date</th><th>Receipt No</th><th>Receive From</th><th>Bank Reference</th><th>Amount</th><th>Status</th><th>View</th></tr>
         </thead>
         <tbody>
             @if(count($payments)>0)
                @foreach($payments as $payment)
                    ...
                    $totaldeposit=$totaldeposit+$payment->deposit_amount;
                    ...
                    <td>{{$payment->deposit_reference}}</td>
                    <td>{{$payment->paid_by}}</td>
                    @if($payment->is_reverse=="0")
                    
                    <td><b>{{number_format($payment->deposit_amount,2)}}</b></td>
                    <td>Ok</td>
                    @else
                    <?php
  ...
  <span style='color:#999'>{{number_format($payment->deposit_amount,2)}}</span></span></td>
                    <td>Canceled</td>
                    @endif
          ...
          <tfoot>
                    ...
                    <th><b>{{number_format($totaldeposit,2)}}</b></th>
                    <th></th></tr>
    ...
    <div class="col-md-3 pull-left">
         <a href="{{url('/cashier',array('print','bank_deposits',$date_from,$date_to))}}" class="btn btn-primary" target="_blank">Print</a>
     </div>   
  </div>  
@endsection
...
<script>
    $(document).ready(function(){
      ...
      $("#view-button").on('click',function(e){
        document.location="{{url('/cashier',array('bank_deposits'))}}" + "/" + $("#date_from").val() + "/" + $("#date_to").val();
    });
