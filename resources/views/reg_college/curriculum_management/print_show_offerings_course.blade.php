...
<div>    
    ...
    <div style='float: left; margin-top:12px; margin-left: 10px' align='center'><span id="schoolname">Assumption College</span> <br><small> San Lorenzo Drive, San Lorenzo Village<br> Makati City</small><br><br><b>MASTER SCHEDULE PER COURSE</b><br><b>A.Y. {{$request->school_year}} - {{$request->school_year + 1}}, {{$request->period}}</b><br></div>
</div>
<div>
    <table class="table table-condensed" width="100%"  style='margin-top: 155px;'>
        <tr>
            <th style="font-size:15pt" align='left'>Course: {{$request->course_code}}</th>
        </tr>    
    </table>
    @if (count($courses)>0)
